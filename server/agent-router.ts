...
const scanInputSchema = z.object({
  ...
  maxMarketDataAgeSeconds: z.number().int().min(1).max(300).default(DEFAULT_RISK_LIMITS.maxMarketDataAgeSeconds),
});

export const agentRouter = router({
  scanCandidates: adminProcedure.input(scanInputSchema).query(async ({ input }) => {
    const limits = {
      ...DEFAULT_RISK_LIMITS,
