...
async function validateExternalOpportunities(
  ...
  try {
    ...
    console.warn("[ArbitrageScanner] LLM validation of external opportunities failed, keeping all:", err);
    return opportunities;
  }

  const parsed = JSON.parse(parseLLMText(result)) as {
    validations?: Array<{ polymarketId: string; kalshiId: string; confidence: number; reason: string }>;
  };
