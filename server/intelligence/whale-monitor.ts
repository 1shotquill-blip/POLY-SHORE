...
async function fetchTopWallets(): Promise<WhaleWallet[]> {
  ...
  const body = (await res.json()) as LeaderboardEntry[] | { data?: LeaderboardEntry[] };
...
function positionToTradePrint(
  ...
  const marketId = String(
    pos.conditionId ?? pos.marketId ?? pos.market ?? ""
  );
...
async function poll(): Promise<void> {
  try {
    ...
    if (Date.now() - lastLeaderboardFetch > 10 * 60_000) {
      ...
      console.log(`[WhaleMonitor] leaderboard refreshed: ${wallets.length} wallets`);
    }
