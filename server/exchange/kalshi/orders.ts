...
import type { AgentMarket, ExecutionReceipt, TradeIntent } from "../../agent/types";
...
function orderIdFrom(body: KalshiOrderResponse): string {
  return String(body.order?.order_id ?? body.order?.id ?? body.order_id ?? body.id ?? "");
}
...
export async function placeKalshiLimitOrder(
  ...
  const body = await client.request<KalshiOrderResponse>("/portfolio/orders", {
    ...
    body: {
      ...
      client_order_id: `kalshi-${now.getTime()}-${nanoid(8)}`,
      side: intent.side === "buy" ? "yes" : "no",
      action: "buy",
      type: "limit",
      count,
      yes_price: intent.outcome === "yes" ? decimalToCents(intent.limitPrice) : undefined,
      no_price: intent.outcome === "no" ? decimalToCents(intent.limitPrice) : undefined,
    },
  });
  const orderId = orderIdFrom(body);
  if (!orderId) {
    return {
      localOrderId: `kalshi-live-${now.getTime()}-${nanoid(8)}`,
      status: "rejected",
      submittedAt: now,
...
export async function cancelKalshiOrder(
  ...
  if (!exchangeOrderId) throw new Error("Cannot cancel Kalshi order without id");
  await client.request(`/portfolio/orders/${encodeURIComponent(exchangeOrderId)}`, {
    method: "DELETE",
  });
  return {
    localOrderId,
