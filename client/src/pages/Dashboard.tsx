    ...
    <div className="space-y-5">
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        {[
          ...
          { label: "All-Time P&L", value: `${usd(data?.pnl.allTimeUsd)} / ${pct(data?.pnl.allTimePct)}`, tone: (data?.pnl.allTimeUsd ?? 0) >= 0 ? "text-emerald-300" : "text-red-300" },
          { label: "Kalshi Balance", value: data?.bankrolls.kalshiUsd == null ? "AUTH OFF" : usd(data.bankrolls.kalshiUsd) },
        ].map(item => (
          ...
          <CardContent>
            <div className="overflow-x-auto">
              <Table>
                <TableHeader>
                  <TableRow className="border-white/10">
                    {["Market", "Token", "Side", "Price", "Size", "USDC Value", "Edge", "Confidence", "Filled At"].map(h => (
                      <TableHead key={h} className="text-xs">{h}</TableHead>
                    ))}
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {(data?.performance.trades ?? []).map(trade => (
                    <TableRow key={trade.id} className="border-white/10">
                      ...
                      <TableCell className="font-mono text-xs text-zinc-400">{compact(trade.tokenId, 6, 4)}</TableCell>
                      <TableCell>
